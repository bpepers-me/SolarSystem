	...
	SubShader {
		...
		Pass {
			...
			vertexOutput vert(vertexInput v) {
				...
				half atmo;
				half light = saturate (dot (normalDir, -lightDir) + .2);
			...
			half4 frag(vertexOutput i) : Color {
				...
				half cloud = tex2D(_CloudTex, i.tex.xy).a * .5;
				tex = lerp(tex, half4(0, 0, 0, 0), cloud);
				
				half4 nightTex = tex2D (_NightTex, i.tex.xy);
				half light = saturate (i.tex.z * 2);
		...
		Pass{
			...
			vertexOutput vert(vertexInput v) {
				...
				half3 viewDir = normalize (_WorldSpaceCameraPos.xyz - mul (unity_ObjectToWorld, v.vertex));
				half3 lightDir = normalize(mul(unity_ObjectToWorld, v.vertex) - _WorldSpaceLightPos0.xyz);
				half light = saturate(min(dot(normalDir, -lightDir), dot(normalDir, viewDir) * 5));
				o.col = _AtmoColor * light;
				o.tex = v.texcoord;
			...
			half4 frag(vertexOutput i) : Color{
				//half4 cloudtex = tex2D(_CloudTex, i.tex.xy) * i.col;
				return tex2D(_CloudTex, i.tex.xy) * i.col;
			}
